...
import { fetchStoreSettings } from '@/lib/queries';
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    async function loadSettings() {
      try {
        const data = profile?.store ?? await fetchStoreSettings(profile?.store_id ?? undefined);
        if (data) {
          setStoreData({
            name: data.name || '',
  ...
  const handleSaveStore = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetchStoreSettings(profile.store_id, { force: true });
    } catch (e: any) {
  ...
  const showSkeletons = profileLoading || loading;
