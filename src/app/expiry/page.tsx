...
import { clearQueryCaches, fetchExpiringBatches } from '@/lib/queries';
import { useUserProfile } from '@/contexts/user-profile-context';
import { createClient } from '@/utils/supabase/client';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { formatExpiryDate, formatCurrency, cn } from '@/lib/utils';
...
export default function ExpiryTracker() {
  ...
  async function fetchData() {
    ...
    setLoading(true);
    try {
      const data = await fetchExpiringBatches(storeId, 180);
      setItems(data);
    } catch (error) {
  ...
  const confirmAction = async () => {
    ...
    try {
      ...
      for (const id of ids) {
        const { error } = await supabase.from('store_inventory_batches').update({ quantity: 0 }).eq('id', id);
  ...
  const selectAll = () => {
    if (selectedBatches.size === items.length) {
      ...
      setSelectedBatches(new Set(items.map((i) => i.batchId)));
    }
  };
  ...
  if (!profileLoading && !storeId) {
    return (
      <div className="container py-8 text-center text-muted-foreground">
        Please select a pharmacy to view expiry data.
      </div>
    );
  }
  ...
  return (
    <div className="container py-4 flex flex-col gap-4 pb-24">
      ...
      <div className="flex justify-between items-center my-2">
        <h2 className="text-xl font-bold tracking-tight">Expiring Batches ({loading ? '…' : items.length})</h2>
      ...
      {!loading && items.length > 0 && (
        <div className="flex items-center gap-2 px-1 mb-2">
          <input type="checkbox" className="w-5 h-5 accent-primary cursor-pointer" checked={selectedBatches.size === items.length} onChange={selectAll} />
          ...
          return (
            ...
            >
              ...
              <CardHeader className="p-4 flex flex-row items-center gap-4 space-y-0">
                <input type="checkbox" className="w-5 h-5 accent-primary shrink-0 cursor-pointer" checked={isSelected} onChange={() => toggleBatch(item.batchId)} />
                <div className="flex-1 grid gap-1">
                  <CardTitle className="text-sm font-bold cursor-pointer" onClick={() => toggleBatch(item.batchId)}>{item.medicineName}</CardTitle>
                  <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Store Section: {item.rack || 'Main'}</p>
                </div>
                <Badge
                  variant={urgency === 'expired' || urgency === 'critical' ? 'destructive' : 'outline'}
                  className={cn(
              ...
              <CardContent className="p-4 pt-0">
                <div className="grid grid-cols-2 gap-y-2 gap-x-3 text-sm font-medium mb-3">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="font-bold">{formatExpiryDate(item.expiryDate)}</span>
                  </div>
                  <div className="flex flex-col">
                ...
                <div className="flex gap-2 pt-4 border-t border-border">
                  <Button variant="outline" size="sm" className="flex-1 font-bold h-9 rounded-lg" onClick={() => requestAction('return', item.batchId)}>
                    <RotateCcw size={12} className="mr-1.5" />
                    Return
                  </Button>
                  <Button variant="outline" size="sm" className="flex-1 font-bold h-9 rounded-lg text-red-500 hover:bg-red-500/10 hover:text-red-600 border-red-100" onClick={() => requestAction('dispose', item.batchId)}>
